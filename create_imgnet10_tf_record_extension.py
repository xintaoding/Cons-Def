...
def create_records(image_dir,file, output_record_dir, resize_height, resize_width,shuffle,log=5):
    ...
    k=1
    writer = tf.python_io.TFRecordWriter(output_record_dir+'_seg'+str(0//1000))
    for i, [image_name, labels] in enumerate(zip(images_list, labels_list)):
      if i%1000==0 and i>1:
        writer = tf.python_io.TFRecordWriter(output_record_dir+'_seg'+str(i//1000))
      ...
      imgs=data_exten(image, 1, nb_classes=10, img_rows=resize_height, img_cols=resize_width)
        
      for n_img in range(imgs.shape[0]):
          image=imgs[n_img,:,:,:]
          image=image.astype(np.uint8)
          if image.shape[0]!=299 or image.shape[1]!=299:
              print("not in size of 299*299")
...
def batch_test(record_file,resize_height, resize_width):
    ...
    # 读取record函数
    tf_image,tf_label = read_records(record_file,resize_height,resize_width,type='normalization')
    image_batch, label_batch= get_batch_images(tf_image,tf_label,batch_size=4,labels_nums=10,one_hot=False,shuffle=False)
...
if __name__ == '__main__':
    ...
    # 产生train.record文件
    image_dir='train'
    train_labels = 'caffe_ilsvrc12/train_first10classes.txt'  # 图片路径
    train_record_output = 'data/record/train{}.tfrecords'.format(resize_height)
...
#    print("save train example nums={}".format(train_nums))
    ...
    # 产生val.record文件
    image_dir='val'
    val_labels = 'caffe_ilsvrc12/val.txt'  # 图片路径
    val_record_output = 'data/record/val{}.tfrecords_65x'.format(resize_height)
    create_records(image_dir,val_labels, val_record_output, resize_height, resize_width,shuffle,log)
#    val_nums=get_example_nums(val_record_output)
#    print("save val example nums={}".format(val_nums))

    # 测试显示函数
    train_record_output = 'data/record/val299.tfrecords_65x'
