...
def create_graph():
  ...
  with tf.gfile.FastGFile(
      'models/caffe_ilsvrc12/resnet50_clean198000_0.7417.pb', 'rb') as f:
...
#      assert 1==2
      ...
      output_name = 'resnet_v1_50/Softmax:0'
...
#      tf.summary.FileWriter('./summary',sess.graph)
      CREATED_GRAPH = True
      ts_nm_list = [tensor.name for tensor in tf.get_default_graph().as_graph_def().node]
      for i in range(len(ts_nm_list)):
          if i%10==0:
              print("i:{}".format(i))
          print(ts_nm_list[i],'\n')
...
#  adv=np.load("imagenet_ResNet50_augmodel_cwl2_10000adv.npy")#save advs

  
  for i in range(val_max_steps):
        print("i:{}".format(i))
        val_x_bat, val_y_bat = sess.run([val_images_batch, val_labels_batch])
#        val_x_bat = np.round(val_x_bat*255)/255.0

        adv_bat = cw.attack(val_x_bat, val_y_bat)
#        adv_bat = np.round(adv_bat*255)/255.0
...
#  np.save("imagenet_ResNet50_augmodel_cwl2_10000adv",adv)#save advs
