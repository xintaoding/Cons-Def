...
#  assert 1==2
  ...
  print("adv sanity_checks passed================")
...
def generate_pgdhead(x, logits,eps=0.1,eps_iter=0.0075,norm_ord=np.inf,clip_min=-0.5,clip_max=0.5,rand_init=1,nb_iter=10):
...
#    pgd = ProjectedGradientDescent(model, sess=sess)
    adv_x_head, attack_label = generate_pgdhead(input_images, out,eps=eps,norm_ord=norm_ord,clip_min=clip_min,clip_max=clip_max,rand_init=rand_init)
    adv_x, grad_x, loss_x, _, _ = fgm(input_images,logits=out,y=y, eps = eps_iter,clip_min=clip_min,clip_max=clip_max)
    ...
    for i in range(val_max_steps):
      ...
      for k in range(nb_iter):
        ...
        advtemp_bat,grad_x_bat = sess.run([adv_x,grad_x], feed_dict=feed_dict)
#        print("grad_x_bat:{},{}".format(np.sum(grad_x_bat),grad_x_bat))
  ...
  #########################################
    coord.request_stop()
    coord.join(threads)
    np.save("cifar10_vgg16_augmodel_pgd_10000adv",adv)#save advs  
