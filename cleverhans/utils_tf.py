...
def model_eval(sess, x, y, predictions, X_test=None, Y_test=None,
  ...
  :param X_test: numpy array with training inputs, input images in batch
  :param Y_test: numpy array with training outputs, ground labels
  :param feed: An optional dictionary that is appended to the feeding
           dictionary before the session runs. Can be used to feed
           the learning phase of a Keras model for instance.
  :param args: dict or argparse `Namespace` object.
  ...
  with sess.as_default():
    ...
    for batch in range(nb_batches):
      ...
      #for untargeted attack, suc_att_exam[i] is true means a successful classified examples, which indicate a failure attack
