...
class CWL2(object):
  def __init__(self, sess, model, batch_size, confidence, targeted,
    ...
    print("learning_rate:{},max_iterations:{},binary_search_steps:{},initial_const:{}".
          format(self.LEARNING_RATE,self.MAX_ITERATIONS,self.BINARY_SEARCH_STEPS,self.initial_const))
...
#    modifier = tf.Variable(tf.truncated_normal(shape, stddev=0.001, dtype=np_dtype))
    ...
    for outer_step in range(self.BINARY_SEARCH_STEPS):
      ...
      for iteration in range(self.MAX_ITERATIONS):
        ...
        _, l, l1, other,real, l2, l2s, scores, nimg, timage = self.sess.run([
            self.train, self.loss, self.loss1, self.others,self.real, self.loss2, self.l2dist, self.output,
            self.newimg, self.timg
        ])
        
#        print("nimg:{}, timg:{}, output:{},batch:{},{}".format(np.sum(nimg),np.sum(timage),np.sum(scores),np.max(batch), np.min(batch)))
#        assert 1==2

#        if iteration % ((self.MAX_ITERATIONS // 10) or 1) == 0:
#          _logger.debug(("    Iteration {} of {}: loss={:.3g},loss1={:.3g},other={},real={}, loss2={:.3g} " +
#                         "l2={:.3g} f={:.3g}").format(iteration, self.MAX_ITERATIONS, l, l1,other, real, l2, np.mean(l2s), np.mean(scores)))
