...
def train(sess, loss, x_train, y_train,
  ...
  print("dataset_train:{},x_train======================:{},{}".format(dataset_train,type(x_train),x_train.shape))
#  assert 1==2   
...
#  assert 1==2
  ...
  pad_size = 3#==============================added by ding, for padding and crop
  x_train = np.pad(x_train,((0,0),(pad_size,pad_size),(pad_size,pad_size),(0,0)),'reflect')
...
#      print("index_shuf:{},{}".format(len(index_shuf),batch_size))
#      assert 1==2
...
#        print("aaaaaaaaaaaaaaaaaaaaaaaaaa")#test for evaluation or not, result: this branch does not evaluated
#        assert 1==2
      else:
...
#      print("=============feed_dict:{}".format(feed_dict))
      ...
      for dev_idx in xrange(num_devices):
        ...
        temp = x_train_shuffled[cur_start:cur_end]#==================added by Ding, for random crop training
        lu1 = np.random.randint(0,6)
        lu2 = np.random.randint(0,6)
        temp = temp[:,lu1:lu1+28,lu2:lu2+28,:]
        feed_dict[xs[dev_idx]] = temp
#        feed_dict[xs[dev_idx]] = x_train_shuffled[cur_start:cur_end]
#        print("feed_dict[xs[dev_idx]]:{}".format(feed_dict[xs[dev_idx]].shape))
#        assert 1==2
  ...
  #    print("train_step:{},loss_value:{}".format((train_step),(loss_value)))
  #    assert 1==2
        
      _, loss_numpy = sess.run(
          [train_step, loss_value], feed_dict=feed_dict)
#      print("Extreme loss during training: {}".format(loss_numpy))
    ...
    if epoch==40 or epoch==45 or epoch==50 or epoch==55 or epoch==60 or epoch==65 or epoch==70 or epoch==75 or epoch==80 or epoch==85 or epoch==90 or epoch==95 or epoch==100:
        saver.save(sess,'./cifar10_clean_train_epoch'+str(epoch))

#    print("=======================evaluate:{},use_ema:{}".format(evaluate,use_ema))
