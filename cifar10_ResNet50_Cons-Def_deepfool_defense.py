...
with tf.Session() as sess:
  ...
  #########################################Added by Ding  
    ...
    saver.restore(sess,'models/cifar10/resnet50_models_2362500_0.9993.ckpt')
    print("aaaaaaaaaaaaaaaaa=************************************")

    adv_x = generate_deepfool(sess, input_images, out, nb_candidate=10,overshoot=10.0,
...
#    adv=np.load("cifar10_ResNet50_augmodel_deepfool_10000adv.npy")
...
#      adv_bat = adv[i*batch_size:(i+1)*batch_size,:,:,:]
  ...
  #########################################
    coord.request_stop()
    coord.join(threads)
    np.save("cifar10_ResNet50_augmodel_deepfool_10000adv",adv)#save advs
