...
def create_graph():
  ...
  with tf.gfile.FastGFile(
      'models/cifar10/resnet50_models_2362500_0.9993.pb', 'rb') as f:
...
#      assert 1==2
      ...
      output_name = 'resnet_v1_50/SpatialSqueeze:0'
    self.img = tf.placeholder(tf.float32, (None, 128,128,3))
...
#    yname = adv_ys
    ...
    self.initial_const = 100.0#'initial_const': 10#10 is the default set of cleverhans,0.001 is the CW author proposed
...
#  adv = np.load("cifar10_ResNet50_augmodel_cwl2_10000adv.npy")#save advs      
...
#  dsae_f=0
  ...
  manager=multiprocessing.Manager()#augment examples parallel, depend the core number of CPU
...
#        loss_adv = sess.run(loss_x,feed_dict=feed_dict)
  ...
  for i in range(len(y_test_ext)):
      ...
      adv_result_stat[temp] = adv_result_stat[temp]+1#there is an inconsensus detection result in the 64 perturbations
