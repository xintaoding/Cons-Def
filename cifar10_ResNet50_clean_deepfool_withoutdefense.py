...
#    np.save("cifar10_ResNet50_augmodel_deepfool_10000adv",adv)#save advs  
    ...
    adv128 = np.zeros((len(adv),26+net_height,26+net_width,3),dtype=np.float32)
    advcrop = np.zeros((len(adv),net_height,net_width,3),dtype=np.float32)
    adv128[:,13:13+net_height,13:13+net_width,:] = adv
    for i in range(len(adv)):
      tf_image = adv128[i,:,:,:]
      lu1 = np.random.randint(0,26)
      lu2 = np.random.randint(0,26)
