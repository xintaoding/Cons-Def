...
#  test_ind = np.arange(0,test_end,10)#np.random.randint(0,x_test.shape[0],source_samples)
...
#  eval_params = {'batch_size': batch_size}
  eval_params = {'batch_size': np.minimum(nb_classes, test_end)}
...
###########################################################################
  ...
  ###########################################################################
  print('Crafting ' + str(test_end) + ' adversarial examples')
  ...
  cw_params = {'binary_search_steps': 1,#'binary_search_steps': 9,#'binary_search_steps': 1,#9 is the CW author proposed, 1 is the cleverhans setting
               ...
               'initial_const': const}#'initial_const': 10#10 is the default set of cleverhans,0.001 is the CW author proposed

  if len(x_test)>1000:
      adv =np.zeros(x_test.shape,dtype=np.float32)
      attack_batch=1000
      n_attack_batch=len(x_test)//attack_batch
      rem_attack = len(x_test)%attack_batch
      for i in range(n_attack_batch):
          adv_input_ys_batch = y_test[i*attack_batch:(i+1)*attack_batch,:]
          cw_params = {'binary_search_steps': 1,#'binary_search_steps': 9,#'binary_search_steps': 1,#9 is the CW author proposed, 1 is the cleverhans setting
               yname: adv_input_ys_batch,#adv_ys
               'max_iterations': attack_iterations,
               'learning_rate': CW_LEARNING_RATE,
               'batch_size': attack_batch,
               'initial_const': const}#'initial_const': 10#10 is the default set of cleverhans,0.001 is the CW author proposed

          adv[i*attack_batch:(i+1)*attack_batch,:,:,:] = cw.generate_np(x_test[i*attack_batch:(i+1)*attack_batch,:,:,:], **cw_params)
      if rem_attack>0:
         rem_begin = i*attack_batch
         rem_end = len(x_test)
         adv[rem_begin:rem_end,:,:,:] = cw.generate_np(x_test[rem_begin:rem_end,:,:,:], **cw_params)
  else:
      adv = cw.generate_np(x_test, **cw_params)
      
#  print("adv----------------------:{},targeted:{}".format(adv.shape,targeted))
  adv_acc,adv_suc_att_exam = model_eval(sess, x, y, preds, adv, y_test, args=eval_params)
  accuracy,suc_att_exam = model_eval(sess, x, y, preds, x_test, y_test, args=eval_params)
  adv_accuracy = 1 - adv_acc 
  print('--------------------------------------')
  np.save("cifar10_DistillationNet_augmodel_cwl2_10000adv",adv)#save advs produced on clean model:
#  adv=np.load("cifar10_DistillationNet_augmodel_cwl2_10000adv.npy")
...
def main(argv=None):
  ...
  cifar10_tutorial(batch_size=FLAGS.batch_size, attack_iterations=FLAGS.attack_iterations)
