...
#val_record_file='../TFML_master/dataset/record/val_fortest299.tfrecords'
...
def create_graph():
  ...
  with tf.gfile.FastGFile(
      'models/caffe_ilsvrc12/imagenet_inceptv3_best_models_682600_0.7458.pb', 'rb') as f:
...
#      assert 1==2
      output_name = "InceptionV3/Logits/SpatialSqueeze:0"
    else:
...
#      output_name = 'InceptionV3/Predictions/Reshape:0'
    ...
    # scaled = (0.5+img)*255
    print("InceptionModel.predict:{},img.shape:{}".format(output_name,img.shape))
...
class CWL2(object):
  def __init__(self, sess, model, batch_size,resize_height, resize_width, depths):
#    yname = adv_ys
    self.sess = sess
    self.confidence = 0
    self.learning_rate = 0.01#5e-3 is the default set of cleverhans, 0.01 is the set of pami paper (the parameters are set the same as deeply supervised discriminative)
    ...
    self.initial_const = 10#0.1#'initial_const': 10#10 is the default set of cleverhans,0.001 is the CW author proposed

    self.CONFIDENCE = 0
    self.batch_size = batch_size
    self.repeat = BINARY_SEARCH_STEPS >= 10
    shape = (batch_size, resize_height, resize_width, depths)
...
#  preds1 = model.model.predict(input_images)
  cw=CWL2(sess, model, batch_size,resize_height, resize_width, depths)#initial cw attach
  ...
  for i in range(val_max_steps):
        ...
        logits_bat = sess.run([preds],feed_dict = {input_images: val_x_bat})
        logits_adv_bat = sess.run([preds],feed_dict = {input_images: adv_bat})
...
#            print("j:{}".format(j))
  ...
  for i in range(len(y_test_ext)):
      ...
      adv_result_stat[temp] = adv_result_stat[temp]+1#there is a inconsensus detection results of the 64 perturbations
