...
The target model is VGG16.
The target model requires the input with the size of 160*160, therefore, the source examples are resized for implementation.
Xintao Ding
...
resize_width = 32
#the size must be suitable with layer in vgg (net, 4096, [5, 5], padding=fc_conv_padding, scope='fc6'), 224*224 corresponds to [7, 7], 160*160 correpsonds to [5, 5], 
...
with tf.Session() as sess:
  ...
  saver.restore(sess,'models/cifar10/vgg16_models_860000+1783000_0.8809.ckpt')
  print("aaaaaaaaaaaaaaaaa=+++++++++++++++++++++++++++++")
#########################################
  adv = np.load("cifar10_ResNet50_augmodel_deepfool_10000adv.npy")#source attacks
...
#      test_image = np.pad(test_image,((pad_size,pad_size),(pad_size,pad_size),(0,0)),'reflect')
  ...
  for i in range(val_max_steps):
      ...
      p1 = multiprocessing.Process(target=data_exten,args=(adv[i*2*batch_size:(2*i+1)*batch_size,:,:,:],
                                                           y_test[i*2*batch_size:(2*i+1)*batch_size,:],
      ...
      p2 = multiprocessing.Process(target=data_exten,args=(adv[(2*i+1)*batch_size:2*(i+1)*batch_size,:,:,:],
                                          ...
                                          batch_size, base_range,labels_nums,net_height,net_width,3,
                                          rt_res_adv2))
      p3 = multiprocessing.Process(target=data_exten,args=(x_test[i*2*batch_size:(2*i+1)*batch_size,:,:,:],
                                                           y_test[i*2*batch_size:(2*i+1)*batch_size,:],
                                          batch_size, base_range,labels_nums,net_height,net_width,3,
                                          rt_res_test1))
      ...
      y_adv_pertpart[i*2*batch_size*n_pert:(2*i+1)*batch_size*n_pert,:] = y_adv_extended1[:batch_size*n_pert,:]
      ...
      y_test_pertpart[i*2*batch_size*n_pert:(2*i+1)*batch_size*n_pert,:] = y_test_extended1[:batch_size*n_pert,:]
      ...
      for j in range(jsteps):
          ...
          feed_dict = {input_images: val_adv_bat, is_training: False}
          logits_adv_bat = sess.run(out, feed_dict=feed_dict)
...
#########################################
  ...
  for i in range(len(y_test_ext)):
      ...
      adv_result_stat[temp] = adv_result_stat[temp]+1
