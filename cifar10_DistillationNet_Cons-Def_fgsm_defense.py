...
#  x_test=np.load("cifar10_DistillationNet_augmodel_fgsmlinf_500x_test_pertpart.npy")
#  test_end=len(x_test)
#  y_test=y_test[:test_end,:]
  adv = np.zeros((test_end, img_rows, img_cols, nchannels),dtype=np.float32)
  grad = np.zeros((test_end, img_rows, img_cols, nchannels),dtype=np.float32)
  loss = np.zeros((test_end),dtype=np.float32)
  val_steps = int(test_end / craft_bs)
  for i in range(val_steps):
      feed_dict = {x: x_test[i*craft_bs:(i+1)*craft_bs,:,:,:]}
      adv[i*craft_bs:(i+1)*craft_bs,:,:,:] = sess.run(adv_x,feed_dict=feed_dict)
      grad[i*craft_bs:(i+1)*craft_bs,:,:,:] = sess.run(grad_x,feed_dict=feed_dict)
      loss[i*craft_bs:(i+1)*craft_bs] = sess.run(loss_x,feed_dict=feed_dict)
      
  np.save("cifar10_x_train_10000",x_train)#save advs produced on clean model:
  np.save("cifar10_DistillationNet_augmodel_fgsmlinf_10000adv",adv)#save advs produced on clean model:
#  np.save("cifar10_DistillationNet_augmodel_fgsmlinf_500adv_pertpart_2ndadv",adv)#save advs produced on clean model:
#  adv=np.load("cifar10_DistillationNet_augmodel_fgsmlinf_10000adv.npy")
#  aug_adv_pertpart_2ndadv_label=np.zeros((test_end,nb_classes),dtype=np.float32)
#  for i in range(val_steps):
#      aug_adv_pertpart_2ndadv_label[i*craft_bs:(i+1)*craft_bs,:]=sess.run(preds, feed_dict={x:adv[i*craft_bs:(i+1)*craft_bs,:,:,:]})
#  aug_adv_pertpart_2ndadv_label=np.argmax(aug_adv_pertpart_2ndadv_label,axis=1)
#  np.save("cifar10_DistillationNet_augmodel_fgsmlinf_500y_2ndadv_pertpart",aug_adv_pertpart_2ndadv_label)

  print("advaaaaaaaaaaaaaaaaaaaaaaaaaaa:{},{},grad:{},loss:{}".format(np.sum(adv-x_test),adv.shape,np.sum(grad),np.average(loss)))
...
#  np.save("cifar10_DistillationNet_augmodel_fgsmlinf_500adv_padcrop",adv[:500,:,:,:])#
#  np.save("cifar10_DistillationNet_augmodel_500test_padcrop",x_test[:500,:,:,:])
